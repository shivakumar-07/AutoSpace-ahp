...
export const SEED_ORDERS = [
    {
        ...
        total: 1850,
        items: "Bosch Brake Pad Set — Front x 1",
        payment: "Prepaid (Escrow)",
        status: "DELIVERED",
        customer: "Rajesh Kumar",
        phone: "9876543210",
        address: "Apt 402, Jubilee Hills, Hyderabad",
        deliveredAt: Date.now() - 1 * 86400000
    },
    {
        id: "ORD-SEEDED-RET",
        shopId: "s1",
        ...
        total: 1400,
        items: "Denso O2 Sensor Universal x 1",
        payment: "Prepaid",
        status: "return_requested",
        customer: "Vikram Singh",
        phone: "9123456780",
        address: "Gachibowli, Hyderabad",
        deliveredAt: Date.now() - 3 * 86400000,
        returnRequest: {
            id: "RET-SEEDED",
            reason: "Defective Product",
            description: "The sensor is not sending any signals to the ECU. Tested with multi-meter.",
            status: 'PENDING',
            createdAt: Date.now() - 1 * 86400000,
            evidenceFile: "defective_sensor.jpg",
            expectedRefund: 1400
        }
    }
];
...
export const RETURN_REASONS = [
    ...
    "Incorrect fitment/compatibility",
    "Changed my mind",
...
// ===== WHATSAPP REMINDER =====
export function generateWhatsAppReminder(name, phone, amount, upiId = "shop@upi") {
    const cleanPhone = (phone || "").replace(/\D/g, "");
    const upiLink = `upi://pay?pa=${upiId}&pn=AutoMobileSpace&am=${Math.round(amount)}&cu=INR&tn=Payment+for+pending+balance`;
    const msg = encodeURIComponent(
        `🙏 Namaskar ${name},\n\nThis is a friendly reminder about your pending balance of ₹${Math.round(amount).toLocaleString("en-IN")} at our shop.\n\n💳 Pay instantly via UPI:\n${upiLink}\n\nPlease settle at your earliest convenience.\nThank you! 🙏`
    );
...
export const calcDeliveryFee = (shopId, customerPincode) => {
    const shopPincodes = { s1: 500033, s2: 500016, s3: 500012 };
    const shopPin = shopPincodes[shopId] || 500001;
    const custPin = parseInt(customerPincode) || 500001;
    const dist = Math.abs(shopPin - custPin);
    if (dist <= 5) return 29;
    if (dist <= 15) return 39;
    if (dist <= 30) return 49;
    return 59 + Math.min(Math.floor(dist / 10) * 5, 40);
};
...
export const ORDER_STATUSES = {
    PLACED: { label: "Order Placed", icon: "📋", color: T.sky, bg: `${T.sky}18`, desc: "Order has been placed and is awaiting seller confirmation.", next: "ACCEPTED", actionLabel: "Accept" },
    ACCEPTED: { label: "Accepted", icon: "✅", color: "#2DD4BF", bg: "rgba(45,212,191,0.12)", desc: "Seller has confirmed the order and is preparing it.", next: "PACKED", actionLabel: "Pack" },
    PACKED: { label: "Packed", icon: "📦", color: T.amber, bg: `${T.amber}18`, desc: "Order has been packed and is ready for dispatch.", next: "SHIPPED", actionLabel: "Ship" },
    SHIPPED: { label: "Shipped", icon: "🚚", color: T.violet, bg: `${T.violet}18`, desc: "Order is on its way to the customer.", next: "DELIVERED", actionLabel: "Delivered" },
    DELIVERED: { label: "Delivered", icon: "📬", color: T.emerald, bg: `${T.emerald}18`, desc: "Order has been delivered to the customer.", next: "COMPLETED", actionLabel: "Complete" },
    COMPLETED: { label: "Completed", icon: "✓", color: T.emerald, bg: T.emeraldBg, desc: "Order is completed successfully.", next: null, actionLabel: null },
    REJECTED: { label: "Rejected", icon: "✕", color: T.crimson, bg: T.crimsonBg, desc: "Order was rejected by the seller.", next: null, actionLabel: null },
    CANCELLED: { label: "Cancelled", icon: "✕", color: T.crimson, bg: T.crimsonBg, desc: "Order was cancelled.", next: null, actionLabel: null },
};

export const ORDER_FLOW = ["PLACED", "ACCEPTED", "PACKED", "SHIPPED", "DELIVERED", "COMPLETED"];

export const ORDER_REJECT_REASONS = [
    "Out of Stock",
    "Price changed, needs re-confirmation",
    "Item discontinued",
...
export const ORDER_ACCEPT_TIMEOUT_MS = 5 * 60 * 1000;

export function normalizeOrderStatus(status) {
    if (status === "NEW") return "PLACED";
    if (status === "DISPATCHED") return "SHIPPED";
    if (ORDER_STATUSES[status]) return status;
    return "PLACED";
}

export function createOrderTimeline(order) {
    const events = order.timeline || [];
    if (events.length === 0 && order.time) {
        return [{ status: "PLACED", timestamp: order.time, note: "Order placed by customer" }];
    }
    return events;
}

export function addTimelineEvent(order, status, note = "") {
    const timeline = createOrderTimeline(order);
    const meta = ORDER_STATUSES[status] || {};
    timeline.push({
        status,
        timestamp: Date.now(),
        note: note || `${meta.label || status}`,
    });
    return timeline;
}

export function getOrderAcceptDeadline(order) {
    const placedEvent = (order.timeline || []).find(e => e.status === "PLACED");
    const placedTime = placedEvent?.timestamp || order.time;
    if (!placedTime) return null;
    return placedTime + ORDER_ACCEPT_TIMEOUT_MS;
}

export function isOrderOverdue(order) {
    if (normalizeOrderStatus(order.status) !== "PLACED") return false;
    const deadline = getOrderAcceptDeadline(order);
    if (!deadline) return false;
    return Date.now() > deadline;
}

export function generateOrderNotification(order, newStatus, extra = {}) {
    const meta = ORDER_STATUSES[newStatus] || {};
    const notifications = {
        PLACED: { title: "New Order Received", message: `Order ${order.id} from ${order.customer || "Customer"} — ${fmt(order.total)}`, type: "info" },
        ACCEPTED: { title: "Order Accepted", message: `Order ${order.id} has been accepted and is being prepared.`, type: "success" },
        PACKED: { title: "Order Packed", message: `Order ${order.id} is packed and ready for dispatch.`, type: "info" },
        SHIPPED: { title: "Order Shipped", message: `Order ${order.id} is out for delivery.`, type: "info" },
        DELIVERED: { title: "Order Delivered", message: `Order ${order.id} has been delivered successfully.`, type: "success" },
        COMPLETED: { title: "Order Completed", message: `Order ${order.id} is now completed.`, type: "success" },
        REJECTED: { title: "Order Rejected", message: `Order ${order.id} was rejected. Reason: ${extra.reason || "N/A"}`, type: "error" },
        CANCELLED: { title: "Order Cancelled", message: `Order ${order.id} was cancelled.`, type: "warning" },
    };
    return notifications[newStatus] || { title: meta.label, message: `Order ${order.id} status: ${meta.label}`, type: "info" };
}

// ===== GST SUMMARY =====
export function getGSTSummary(movements, cutoff) {
    const sales = movements.filter(m => m.type === "SALE" && m.date >= cutoff);
    const purchases = movements.filter(m => m.type === "PURCHASE" && m.date >= cutoff);
    const outputGST = sales.reduce((s, m) => s + (m.gstAmount || 0), 0);
    const inputGST = purchases.reduce((s, m) => s + (m.gstAmount || 0), 0);
    const netLiability = outputGST - inputGST;
    return { outputGST, inputGST, netLiability };
}

// ===== DEAD STOCK (180+ days no sale) =====
export function getDeadStock(products, movements, dayThreshold = 180) {
    const now = Date.now();
    const DAY = 86400000;
    return products.filter(p => {
        ...
        const daysSinceLastSale = Math.floor((now - lastSale.date) / DAY);
        return daysSinceLastSale >= dayThreshold;
    }).map(p => {
        const lastSale = movements
            .filter(m => m.productId === p.id && m.type === "SALE")
            .sort((a, b) => b.date - a.date)[0];
        const daysSinceLastSale = lastSale ? Math.floor((now - lastSale.date) / DAY) : Infinity;
        const capitalTied = p.buyPrice * p.stock;
        return { ...p, daysSinceLastSale, capitalTied };
    });
}

// ===== ONLINE VS OFFLINE BREAKDOWN =====
export function getChannelBreakdown(movements, cutoff) {
    const sales = movements.filter(m => m.type === "SALE" && m.date >= cutoff);
    const online = sales.filter(m => m.channel === "online" || m.source === "marketplace");
    const offline = sales.filter(m => !m.channel || m.channel === "offline" || m.source !== "marketplace");
    return {
        online: { count: online.length, revenue: online.reduce((s, m) => s + m.total, 0), profit: online.reduce((s, m) => s + (m.profit || 0), 0) },
        offline: { count: offline.length, revenue: offline.reduce((s, m) => s + m.total, 0), profit: offline.reduce((s, m) => s + (m.profit || 0), 0) },
    };
}
