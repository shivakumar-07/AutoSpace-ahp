...
import { fmt, uid, calcDeliveryFee } from "../../utils";
...
export function CheckoutPage({ onBack, onOrderPlaced }) {
    ...
    const cartByShop = useMemo(() => {
        return safeCart.reduce((acc, item) => {
            ...
            if (!acc[shopId]) {
                acc[shopId] = { shopId, shop: item.listing.shop, items: [], subtotal: 0, shipping: calcDeliveryFee(shopId, address.pincode) };
            }
            acc[shopId].items.push(item);
            acc[shopId].subtotal += (item.listing?.selling_price || 0) * item.qty;
            return acc;
        }, {});
    }, [safeCart, address.pincode]);
    ...
    const handlePlaceOrder = async () => {
        ...
        safeCart.forEach(item => {
            ...
            if (!p || p.stock <= 0) {
                const alts = safeProducts.filter(alt =>
                    alt.stock > 0 && alt.name === (p?.name || item.product?.name) && alt.shopId !== item.listing?.shop_id
                );
                issues.push({ item, reason: "Sold Out", alternatives: alts });
            } else if (p.stock < item.qty) {
                issues.push({ item, reason: `Only ${p.stock} available (you need ${item.qty})`, alternatives: [] });
            }
        });
        ...
        Object.values(cartByShop).forEach(group => {
            ...
            newOrders.push({
                ...
                vehicle: null,
                timeline: [{ status: "PLACED", time: now, note: "Order placed by customer" }]
            });
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: 40 }}>
                    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                        {stockIssues.length > 0 && (
                            <div style={{ background: `${T.crimson}15`, border: `1px solid ${T.crimson}44`, borderRadius: 16, padding: 24 }}>
                                <div style={{ fontSize: 16, fontWeight: 800, color: T.crimson, marginBottom: 12 }}>⚠ Stock Issues Detected</div>
                                {stockIssues.map((issue, i) => (
                                    <div key={i} style={{ padding: "10px 0", borderTop: i > 0 ? `1px solid ${T.border}` : "none" }}>
                                        <div style={{ fontSize: 13, fontWeight: 700, color: T.t1 }}>{issue.item.product?.name}</div>
                                        <div style={{ fontSize: 12, color: T.crimson, marginTop: 2 }}>{issue.reason}</div>
                                        {issue.alternatives.length > 0 && (
                                            <div style={{ marginTop: 8 }}>
                                                <div style={{ fontSize: 11, color: T.sky, fontWeight: 700, marginBottom: 4 }}>Available from other sellers:</div>
                                                {issue.alternatives.map((alt, ai) => (
                                                    <div key={ai} style={{ fontSize: 12, color: T.t2, padding: "2px 0" }}>
                                                        {alt.name} @ {fmt(alt.sellPrice)} — {alt.stock} in stock
                                                    </div>
                                                ))}
                                            </div>
                                        )}
                                ...
                                <div style={{ fontSize: 12, color: T.t3, marginTop: 12 }}>Please go back and update your cart, then try again.</div>
                            </div>
                        )}
                            ...
                            <div key={idx} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24 }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                                        <span style={{ fontSize: 12, color: T.sky, fontWeight: 600 }}>🚚 ~45 min</span>
                                        <span style={{ fontSize: 11, color: T.t3, fontFamily: FONT.mono }}>Del: {fmt(group.shipping)}</span>
                                    </div>
                                </div>
                    ...
                    <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 16, padding: 24, position: "sticky", top: 20, alignSelf: "start" }}>
                        ...
                        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", fontSize: 13, color: T.t2 }}><span>Delivery ({Object.keys(cartByShop).length} shipment{Object.keys(cartByShop).length !== 1 ? "s" : ""})</span><span style={{ fontFamily: FONT.mono }}>{fmt(totalShipping)}</span></div>
                        ...
                        <button
                            ...
                            style={{ marginTop: 24, width: "100%", background: stockIssues.length > 0 ? T.t3 : T.amber, color: "#000", border: "none", borderRadius: 12, padding: "16px", fontSize: 16, fontWeight: 900, cursor: stockIssues.length > 0 ? "not-allowed" : "pointer", boxShadow: stockIssues.length > 0 ? "none" : `0 8px 28px ${T.amber}55`, display: "flex", alignItems: "center", justifyContent: "center", gap: 8 }}
                        >
            ...
            {step === "success" && (
                <div style={{ textAlign: "center", padding: "60px 20px" }}>
                    ...
                    <div style={{ fontSize: 15, color: T.t2, marginBottom: 24 }}>
                        {orderIds.length} shipment{orderIds.length > 1 ? "s" : ""} from {orderIds.length} shop{orderIds.length > 1 ? "s" : ""}
                    </div>
